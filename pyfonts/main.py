...
from pyfonts.utils import _get_font_provider_metadata, _get_fonturl
...
def load_font(
    ...
    if font_url is not None:
        if not _is_url(font_url):
            ...
            font_prop: FontProperties = FontProperties(fname=font_url)
            try:
                font_prop.get_name()
            except FileNotFoundError:
...
def set_default_font(font: FontProperties) -> None:
    """
    Set the default font for all text elements generated by matplotlib,
    including axis labels, tick labels, legend entries, titles, etc.
    ...
    Examples:
        ...
        ```
    """
    _register_font_variants(font)
    rcParams.update(
        {
            "font.family": _get_default_font_families(font),
...
def _get_default_font_families(font: FontProperties) -> list[str]:
    families: list[str] = [font.get_name()]
    provider_metadata = _get_font_provider_metadata(font)
...
def _register_font_variants(font: FontProperties) -> None:
    for font_file in _get_font_variant_files(font):
        fontManager.addfont(font_file)


def _get_font_variant_files(font: FontProperties) -> set[str]:
    font_files: set[str] = set()
    current_font_file = font.get_file()

    if current_font_file is not None:
        font_files.add(str(current_font_file))

    provider_metadata = _get_font_provider_metadata(font)
    if provider_metadata is None:
        return font_files

    # Best-effort preload of provider variants so matplotlib can honor
    # later weight/style overrides against the same family.
    for italic in (False, True):
        for weight in range(100, 1000, 100):
            ...
            if variant_file is not None:
                font_files.add(str(variant_file))
